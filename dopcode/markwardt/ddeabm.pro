...
;      May 2002, CM
;    Fix two errors in the DINTP interpolation step, 04 Jul 2002, CM
;    Handle case of IMPULSES more correctly, 25 Sep 2002, CM
;
;
;  $Id: ddeabm.pro,v 1.1.1.1 2003/06/12 21:43:41 johnjohn Exp $
;-
; Portions Copyright (C) 2002, Craig Markwardt
...
function ddeabm_func0n, func, a, y, private, _extra=fa
  common ddeabm_func_common, nfev
  nfev = nfev + 1
  return, call_function(func, a, y)
end

function ddeabm_func1n, func, a, y, private, _extra=fa
  common ddeabm_func_common, nfev
  nfev = nfev + 1
  return, call_function(func, a, y, private)
end

function ddeabm_func0e, func, a, y, private, _extra=fa
  common ddeabm_func_common, nfev
  nfev = nfev + 1
  return, call_function(func, a, y, _extra=fa)
end

function ddeabm_func1e, func, a, y, private, _extra=fa
  common ddeabm_func_common, nfev
  nfev = nfev + 1
  return, call_function(func, a, y, private, _extra=fa)
end

; *DECK DHSTRT
pro DHSTRT, DF, NEQ, A, B, Y, YPRIME, ETOL, MORDER, SMALL, $
...
; *DECK DDES
      ...
      pro ddes, DF, NEQ, T, Y, TOUT, INFO, RTOL, ATOL, IDID, $
...
; C
      ...
      IF (T NE TOUT) THEN GOTO, DDES_220
      IDID=2
      YPOUT = YP
      TOLD=T
      RETURN
...
; C
      ...
      H = MAX([FOURU*ABS(X),ABS(TOUT-X)]) * ( (TOUT GE X)?(+1):(-1) )
; C
; C.......................................................................
; C
...
; C
      ...
      IF (ABS(X-T) GE ABSDEL) THEN BEGIN
          DINTP, X,YY,TOUT,Y,YPOUT,NEQ,KOLD,PHI,IVC,IV,KGI,GI, $
                                             ALPHA,G,W,XOLD,P
          IDID = 3
...
; C
  DDES_420:
      KLE4 = KLE4 + 1
      IF(KOLD GT 4) THEN KLE4 = 0
      IF(KLE4 GE 50) THEN STIFF = 1
      INTOUT = 1
      GOTO, DDES_250
    END
...
; *DECK DINTP
pro DINTP, X, Y, XOUT, YOUT, YPOUT, NEQN, KOLD, PHI, IVC, $
...
; C
      ...
      DHSTRT, DF,NEQN,X,X+H,Y,YP,WT,1,U,BIG, $
...
; C
      ERKP1 = 0.0D0
      IF (KNEW EQ KM1  OR  K EQ 12) THEN PHASE1 = 0
...
pro DDEABM, DF, T, Y, TOUT0, PRIVATE, FUNCTARGS=fa, STATE=state, $
            ...
            TGRID=tgrid, YGRID=ygrid, YPGRID=ypgrid, NGRID=nsamp, $
            TIMPULSE=timpulse, YIMPULSE=yimpulse, $
            NFEV=nfev, errmsg=errmsg
...
; C***FIRST EXECUTABLE STATEMENT  DDEABM
      ...
      common ddeabm_func_common, ddeabm_nfev
      ddeabm_nfev = 0L
      nfev = 0L
      ...
      INFO = [ 1 - KEYWORD_SET(INIT0), n_elements(rtol) GT 1, $
      ...
      IF ( INFO(1-1) EQ 0 ) OR N_ELEMENTS(STATE) EQ 0 THEN BEGIN
          STATE = {YPOUT: dblarr(neq), TSTAR: 0D, YP: dblarr(NEQ), $
                   ...
                   INTOUT: 0L, NS: 0L, KORD: 0L, KOLD: 0L, INIT: 0L, $
...
; C
     ...
     while (i LT ngrid) do begin
         if keyword_set(intermediate) then begin
             ...
             if ki LT nimpulse then begin
                 if abs(timpulse(ki)-t) LT abs(tout-t) then begin
                     TOUT = TIMPULSE(ki)
                     doimpulse = 1
                 endif
             endif
         ...
         DDES, DF,NEQ,T,Y,TOUT,INFO,RTOL,ATOL,IDID,YPOUT, $
           YP,YY,WT,P,PHI, $
           ALPHA,BETA,PSI,V, $
           W,SIG,G,GI,H, $
           EPS,X,XOLD,HOLD, $
           TOLD,DELSN,TSTOP,TWOU, $
           FOURU,START,PHASE1,NORND,STIFF,INTOUT, NS, KORD, KOLD, INIT, $
           KSTEPS, KLE4, IQUIT, KPREV, IVC, IV, KGI, PRIVATE, FA, dfname, $
           ERRMSG=errmsg

         if IDID GT 0 then begin
             if doimpulse then begin    
                 ;; Apply an impulse, but don't store the  result
                 Y = Y + YIMPULSE(*,ki)
                 doimpulse = 0
                 init = 0L       ;; Reset the integration to begin freshly
                 info(1-1) = 0L
                 ki = ki + 1
             endif else begin
                 ;; Store the result
