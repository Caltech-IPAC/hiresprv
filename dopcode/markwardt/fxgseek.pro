...
;
;  $Id: fxgseek.pro,v 1.1.1.1 2003/06/12 21:43:40 johnjohn Exp $
;
;-
...
@fxfilter
  ...
  point_lun = 'POINT_LUN'
  if filterflag(unit0) EQ 1  then $
    if seek_cmd(unit0) NE '' then point_lun = seek_cmd(unit0)
  if point_lun EQ '-' then $
    message, 'ERROR: This resource does not support seeking.'
