...
;
;  $Id: fxpbuffr.pro,v 1.1.1.1 2003/06/12 21:43:40 johnjohn Exp $
;
;-
...
@fxpcommn
  ...
  ;; size.
  NTOT = CEIL(DOUBLE(NEWLEN-CACHE_LEN(UNIT))/BUFFER_GRAN) * BUFFER_GRAN

  IF NOT EOF_REACHED(UNIT) THEN $
    POINT_LUN, CACHE_UNIT(UNIT), CACHE_LEN(UNIT)
  ...
  WHILE NOT EOF_REACHED(UNIT) AND NTOT GT 0 DO BEGIN
      ...
      IF CC GT 0 THEN WRITEU, CACHE_UNIT(UNIT), BUFFER(0:CC-1)
  ...
  POINT_LUN, CACHE_UNIT(UNIT), POINTER(UNIT)
