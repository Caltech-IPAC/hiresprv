...
;
;  $Id: fxpopenr.pro,v 1.1.1.1 2003/06/12 21:43:40 johnjohn Exp $
;
;-
...
@fxfilter
  ...
  FOR I = 0L, N-1 DO BEGIN
      ...
      IF NOT (CC EQ ' ' OR CC EQ '>' OR CC EQ '&' OR CC EQ '|' OR $
              CC EQ '/' OR CC EQ '*' OR CC EQ '?' OR CC EQ '<') THEN $
        NEWNAME = NEWNAME + CC
  ENDFOR
...
;; Main entry
PRO FXPOPENR, UNIT, CMD, ERRMSG=ERRMSG, ERROR=error
...
;  EXEC = [SHELL, NEWCMD]
;  SPAWN, EXEC, /NOSHELL, UNIT=UNIT, PID=PID
  EXEC = NEWCMD
  SPAWN, EXEC, UNIT=UNIT, PID=PID
  ON_IOERROR, NULL
  ...
  OPENW, CACHE, CACHE_FILENAME, /GET_LUN
  ON_IOERROR, NULL
