...
;
;  $Id: hprnutang.pro,v 1.1.1.1 2003/06/12 21:43:41 johnjohn Exp $
;
;-
...
pro hprnutang_init_iau1980_args, args
  ...
  ;; c3 = mean longitude of the Moon minus the mean longitude of
  ;; Moon's node
...
pro hprnutang, jdtt, zeta, theta, z, dpsi, deps, jpl=jpl, $
               ...
               use_eopdata=useeop, no_ut1=no_ut1
...
; if keyword_set(arg96) then farg = farg96 else farg = farg80
  ...
  else                            tbase = double(tbase0(0))

  if n_elements(fbase0) EQ 0 then fbase = 0d $
  else                            fbase = double(fbase0(0))
  ...
  if keyword_set(jpl) then begin
      ...
      jdlimits = [min(jdtt)-1, max(jdtt)+1]+tbase
