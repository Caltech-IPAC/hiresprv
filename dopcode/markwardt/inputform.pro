...
;   objects cannot be printed.  [ However, pointers *can* be
...
;
;  $Id: inputform.pro,v 1.1.1.1 2003/06/12 21:43:40 johnjohn Exp $
;
;-
; Copyright (C) 2000,2001,2002, Craig Markwardt
...
function inputform_ptr, x
  ...
  format = '('+strtrim(nel,2)+'("ptr_valid(",I0,",/cast)",:,","))'
...
function inputform, data, errmsg=errmsg, status=status, max_elements=nmax, $
  ...
  ndims = sz(0)
  tp = sz(ndims+1)
  if (tp EQ 0) OR (tp EQ 11) OR (tp GT 15) then begin
