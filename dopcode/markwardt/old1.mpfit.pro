...
;
;  $Id: mpfit.pro,v 1.1.1.1 2003/06/12 21:43:39 johnjohn Exp $
;-
...
;; derivatives or not.
function mpfit_call, fcn, x, fjac, _EXTRA=extra, result=f
...
function mpfit_fdjac2, fcn, x, fvec, step, ulimited, ulimit, dside, $
  ...
  if NOT keyword_set(autoderiv) then begin
      ...
      fp = mpfit_call(fcn, xall, fjac, _EXTRA=fcnargs, result=result)
      iflag = mperr
...
pro mpfit_defiter, fcn, x, iter, fnorm, FUNCTARGS=fcnargs, $
  ...
  print, iter, fnorm/dof, dof, $
    format='("Iter ",I6,"   REDUCED CHI^2 = ",G15.8,"          DOF = ",I0)'
...
function mpfit, fcn, xall, FUNCTARGS=fcnargs, SCALE_FCN=scalfcn, $
                ...
                niter=iter, nfree=nfree, npegged=npegged, $
...
;  prof_start = systime(1)
  ...
  if NOT isext then begin
      ...
      fjac = mpfit_fdjac2(fcn, x, fvec, step, qulim, ulim, dside, $
                          iflag=iflag, epsfcn=epsfcn, result=result, $
  ...
  if keyword_set(miniter) then if iter lt miniter then info = 0
