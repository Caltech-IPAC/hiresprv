...
;
;  $Id: plotbin.pro,v 1.1.1.1 2003/06/12 21:43:40 johnjohn Exp $
;
;-
...
pro plotbin, x0, y0, width=width, pixcenter=pixcenter, $
  ...
  if n_elements(panel) EQ 0 AND n_elements(subpanel) EQ 0 then begin
      plot, xtop, ytop, _EXTRA=extra
  endif else begin
      if n_elements(panel) EQ 0 then panel=[0.0,0.0,1.0,1.0]
      plot, xtop, ytop, /normal, position=subcell(subpanel, panel, /marg), $
        _EXTRA=extra
  endelse
