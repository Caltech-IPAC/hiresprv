FUNCTION ROBUST_POLY_FIT,X,Y,NDEG,YFIT,SIG, NUMIT=THIS_MANY, wgt=wgt
;+
; NAME:
;	ROBUST_POLY_FIT
;
...
IF NDEG LT DEGMAX THEN BEGIN
   ...
   U=double(X)-double(X0)            &  V=double(Y)-double(Y0)
ENDIF ELSE BEGIN
...
;  Settle for least-squares:
   LSQFIT = 1
   CC = POLYFIT( U, V, NDEG, YFIT, weight=wgt )
ENDIF ELSE BEGIN
...
;  Put the leftover points in the middle segment:
   ...
   ENDFOR
;  Now fit:
   CC = POLYFIT( R,S, NDEG, weight=wgt )
   YFIT = POLY(U,CC)  
ENDELSE
...
;  CC= POLYFITW( U, V, W, NDEG, YFIT )
  CC= POLYFIT( U, V, NDEG, YFIT, weight=W )
  ISTAT=ROB_CHECKFIT(V,YFIT,EPS,DEL,  SIG,FRACDEV,NGOOD,W,S)
  IF ISTAT EQ 0 THEN GOTO,AFTERFIT
  IF NGOOD LT MINPTS THEN BEGIN
