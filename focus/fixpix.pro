...
;               Dewar #101 is for Keck Hi-Res Data prior to rk5
;               Dewar #102 is for Keck Hi-Res Data for rk5 and later
;
...
;kludge fix for negative normalization
   if dewar eq 39 or dewar ge 101 then begin
      ind=where(dum lt 0,nind)
      if nind gt 0 then dum(ind)=dum(ind)+65536.0
   endif

;normalization for dewars 39, 98, 99, 101, 102
   if (dewar eq 39) or (dewar eq 101) or (dewar eq 102) $
    or (dewar eq 98) or (dewar eq 99) then begin
      for n=0,d do im(*,n)=im(*,n)*(1000./dum(n))
      im(ncol-1,*)=im(ncol-3,*)
   endif 

;normalization for dewars 1, 2, 6, 8, 13
   if not keyword_set(old) and (dewar ne 39) and (dewar ne 101) and (dewar ne 102) $
       and (dewar ne 98) and (dewar ne 99) then begin
