...
;if keyword_set(mark) then begin 
  ...
  maxgray = 5000
  maxgray = 1500
  mingray = median(im(1500:2500,1000:1200))
  maxgray = mingray + 100.
  display, im, xs, ys, min = mingray, max = maxgray, titl = titl, ytit = yt, xtit = xt, /psfine
  oplot, [0, 4096], [2200, 2200]
  oplot, [0, 4096], [4250, 4250]
  window, 0, xsize = 1000, ysize = 1200, title = 'Planet-Hunting PSF Focus'
  !p.charsize=1.3
  display,im,xs,ys,min=mingray,max=maxgray,titl=titl,ytit=yt,xtit=xt,/psfine
  oplot,x,y+2,ps=6,symsize=2,thick=1,col=255 ;boxes at LINE LOC's (+1 is kludge)
...
loadct,39
    IF keyword_set(plt) then begin  ;plot each line fit.
      plot,ind,mashrow,ps=4,yr=[0,1.2],/ysty,syms=1.8, col=0
...
; print,format='(A25,A6)','      CCD Position =',strtrim(string(fix(focus)),2)
 ...
 print,'--------------------------------------'
 print,format='(A15,F8.3,A14)','AVG FWHM =  ',avgfwhmx,'   2.30 - 2.42)'
...
;  print,'   Move Echelle Grating Angle: ',(15./24.)*0.001*avgdx, ' deg (upgraded HIRES) .'
  print,'   Move Echelle Grating Angle: ',0.00070*avgdx, ' deg (upgraded HIRES) .'
  end else begin
  print,''
...
;  print,'          Move X-Dispersor Angle: ',0.0023*avgdy,' degrees.'
  ...
  print,''
  print,'  No X-disperser  move needed.'
  print,' '    
 endelse
