...
;  display,im,xs,ys,min=890,max=1700,titl=titl,ytit=yt,xtit=xt
  oplot,x,y+2,ps=6,symsize=1.8,thick=1 ;boxes at LINE LOC's (+1 is kludge)
...
;
 if abs(avgdx) gt 1 then begin
  ...
  print,'*******************************************************************'
  print,'         Move Grating Angle: ',0.001*avgdx, ' degrees  (HIRES) .'
  end else begin
  print,''
