; $Id: //depot/Release/ENVI51_IDL83/idl/idldir/lib/read_ascii.pro#1 $
;
; Copyright (c) 1996-2013, Exelis Visual Information Solutions, Inc. All
...
function ra_split_csv, lineIn
  ...
  ; If there are no strings in the line, just split on commas and return.
  if (ARRAY_EQUAL(isQuote, 0b)) then begin
    return, STRTOK(lineIn, STRING(44b), /EXTRACT, /PRESERVE_NULL)
  endif

  ; 34b is a doublequote "
  ; 44b is a comma ,
  for i=0,n-1 do begin
    if (isQuote[i]) then begin  ; doublequote
      if (withinString) then begin
        ; Did we find an escaped ", which is ""?
        if ((i+1) lt n && isQuote[i+1]) then begin
          ; If so, just convert it to one "
          a[len++] = 34b
          i++
        endif else begin
          ; If not, then this is the end of the string. Swallow the "
          withinString = 0b
        endelse
      endif else begin
        ; Start of a string. Swallow the "
        withinString = 1b
      endelse
    endif else if (b[i] eq 44b) then begin  ; comma
      ; Keep commas within strings, otherwise mark with a special character
      a[len++] = withinString ? 44b : 254b
    endif else begin
      a[len++] = b[i]
    endelse
  endfor

  line = STRING(a[0:len-1])

  ; Split on our special character
  return, STRTOK(line, STRING(254b), /EXTRACT, /PRESERVE_NULL)
