...
; 4-dec-2009 HTI   Reduced threshold for exiting while loop
...
for icol = 0, ncol-1 do begin
  ...
  if (ncore lt 1) then begin 
    ...
    good[icol] = 1 
                                ...
                                ;poorer guess, if all else fails
...
;  igd = where ((abs(int[0, ido]-prevint[0, ido])-iint*1.0e-4) lt 0., ngood)
  igd = where ((abs(int[0, ido]-prevint[0, ido])-iint*3.0e-4) lt 0., ngood) ;hti
...
;rep = where(potcos gt 0. and potcos lt 3., nrep)
rep = where(potcos gt 0. and potcos le 3., nrep) ;hti 4 Dec 2009
