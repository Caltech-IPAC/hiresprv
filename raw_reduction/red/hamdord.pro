...
;06-NOV-09 GM, HI, default orders are now passed as keywords, not saved.
...
; open environmnet variable to identify sacred orders
working_dir = getenv("RAW_RED")

;Read in and mask image.
hiraw,im,ordfname,chip=3   ; hires mosaic
...
;      biaslev = median(im[21+2047+11:21+2047+11+20,j]) ; bias from 2079:2073+25
      biaslev = median(im[*,5:10])  ;hires mosaic
      im[*,j] = im[*,j] - biaslev
    end
...
; HTI, removed this if statement and restores sacred orders if fail.
  if not keyword_set(orc) then begin			;Did fords fail?
;   message,'Unable to find default order locations - aborting.'
     message,'Unable to find default order locations - reverting to saved order locations'
     restore, raw_red +'sacred_orc_red.dat'
     ; restores orc, which is polynomial values of order locations.

  end else begin
  
	trace, 10, 'HAMDORD: order locations are passed through reduction.'
