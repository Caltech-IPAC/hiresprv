...
;  range = 8                       ;vertical search range in rows (+-)
 	range = 10 ;hti 19nov2009
  if ham_bin eq 2 then range = 4
;  shft = findgen(range*2 + 1)-range  ;Amount of shift of rows for each index
  shft = findgen(range*2 + 1)-(range + 4) ; instead of 10, added 2 to account for large shifts. range is effectively -12 to 6
...
print,'----------------------'
 FOR onum = 0,nord-1 do begin                ;loop thru orders    
   ...
   if abs(shft[indmax]) ge range+4 then begin
;   print,'SHIFTORC: Apparent shift is',shft[indmax],' --- outside range,',range
     print,'SHIFTORC: Apparent shift is',shft[indmax],' --- outside range,',range +4  ;added 2 to re-align with actual shift(see shft defined above)
